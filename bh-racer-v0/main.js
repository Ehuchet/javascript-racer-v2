...
var timeScales = [];
timeScales.push( // timeScale 0 - space, largest scale
...
timeScales.push( // timeScale 1 - space, next largest scale
    {   bg1: BACKGROUND.GALAXY,
        bg1Speed: 0.0003,       // background layer scroll speed when going around curve (or up hill)
        bg1Offset: 0,            // current scroll offset for background 1
        allowRotate: false,
...
timeScales.push( // timeScale 2 - desert world, no road, sandy ground
    {   bg1: BACKGROUND.SKY,
...
timeScales.push( // timeScale 3 - water world, no road, blue "ground"
    {   bg1: BACKGROUND.SKY,
        bg1Speed: 0.0005,       // background layer scroll speed when going around curve (or up hill)
        bg1Offset: 0,           // current scroll offset for background 1
        allowRotate: false,
...
timeScales.push( // timeScale 4 - regular world, grass "ground"
    {   bg1: BACKGROUND.SKY,
        bg1Speed: 0.0005,       // background layer scroll speed when going around curve (or up hill)
        bg1Offset: 0,           // current scroll offset for background 1
...
timeScales.push( // timeScale 5 - regular world, grass "ground"
    {   bg1: BACKGROUND.SKY,
        bg1Speed: 0.0005,       // background layer scroll speed when going around curve (or up hill)
        bg1Offset: 0,           // current scroll offset for background 1
        bg2: BACKGROUND.HILLS,
...
timeScales.push( // timeScale 6 - regular world, grass "ground"
    {   bg1: BACKGROUND.SKY,
        bg1Speed: 0.0005,       // background layer scroll speed when going around curve (or up hill)
        bg1Offset: 0,           // current scroll offset for background 1
        bg2: BACKGROUND.HILLS,
...
timeScales.push( // timeScale 7 - regular world, grass "ground"
    {   bg1: BACKGROUND.SKY,
        bg1Speed: 0.0005,       // background layer scroll speed when going around curve (or up hill)
        bg1Offset: 0,           // current scroll offset for background 1
        bg2: BACKGROUND.HILLS,
...
timeScales.push( // timeScale 8 - regular world, grass "ground"
    {   bg1: BACKGROUND.SKY,
        bg1Speed: 0.0005,       // background layer scroll speed when going around curve (or up hill)
        bg1Offset: 0,           // current scroll offset for background 1
        bg2: BACKGROUND.HILLS,
...
function createCircle() {
    ...
    if (timeScaleJump > 0) { // create a bigger/whiter explosion for timescale jumps
        size = 4;
        rmax = 30;
        alpha = .75;
    }
...
function createFragment() {
    var fsize = .005; //default starting %size
    var maxsize = 2.5; //default maximum %size
    this.a = .5 * Math.random(); //random starting alpha
    this.va = .0005 + Math.random() / 100; // alpha decrease factor
    this.vx = -20 + Math.random() * 40; // pace at which the sprite moves away from the centre
    this.vy = -20 + Math.random() * 40;
    this.vs = 1 + Math.random()*.1; // speed of sprite

    if (timeScaleJump > 0) { // create a bigger/whiter explosion for timescale jumps
        fsize = .003;
        maxsize = 2.5;
        this.a = .5 * Math.random(); //random starting alpha
        this.va = .0005 + Math.random() / 100;
        this.vx = -15 + Math.random() * 30;
        this.vy = -15 + Math.random() * 30;
        this.vs = 1 + Math.random()*.08;
    }
...
//    updateCars(dt, playerSegment, playerW);
    ...
    updateHud('smoothness',Math.round((position+(timeBackward?0:trackLength))/10000/keyCount)); // test
    ...
    if (playerSegment.timeMarker)
        updateHud('time_marker',timeMarkerVal(timeScale, playerSegment.timeMarker)); // test
        //Dom.show('time_marker');

    // new test for onroad sprite collision
    for (n = 0; n < playerSegment.sprites.length; n++) { //need to chg to use special array for stationary sprites that excludes off-road ones
        sprite = playerSegment.sprites[n];
        if (sprite.roadSprite) {
            ...
            if (Util.overlap(playerX, playerW, spriteX, spriteW, .8)) {
                ...
                if (sprite.timeJump) {
                    ...
                    if (timeScaleJump <= 0) { //time scale jump
                        timeScaleJump = .98;
                ...
                } else if (sprite.seedSprite) { // create flash when hitting seed, remove seed sprite from segment
                    //noinspection JSDuplicatedDeclaration
    ...
    currentTimeScale.bg1Offset = Util.increase(currentTimeScale.bg1Offset, currentTimeScale.bg1Speed * playerSegment.curve * (position - startPosition) / segmentLength, 1);
    currentTimeScale.bg2Offset = Util.increase(currentTimeScale.bg2Offset, currentTimeScale.bg2Speed * playerSegment.curve * (position - startPosition) / segmentLength, 1);
    currentTimeScale.bg3Offset = Util.increase(currentTimeScale.bg3Offset, currentTimeScale.bg3Speed * playerSegment.curve * (position - startPosition) / segmentLength, 1);
...
//      updateHud('speed',            5 * Math.round(speed/500));
    updateHud('speed', Math.round(5 * Math.round(speed / 500) / maxSpeed * 10000));
...
//updateHud('scale_factor',timeScale); // test
    ...
    if (bike && currentTimeScale.allowRotate) {
        ...
        if (baseSegment.curve == 0) {
            ...
            var newrot  = Util.limit(Math.round(baseSegment.curve * speed / maxSpeed * 1000) / 1000, -6, 6);
...
//					rotationFrame.style.transform = "rotate(" + rotation + "deg)";
    ...
    Render.background(ctx, background, width, height, currentTimeScale.bg1, currentTimeScale.bg1Offset, resolution * currentTimeScale.bg1Speed * playerY);
    if (currentTimeScale.bg2)
        Render.background(ctx, background, width, height, currentTimeScale.bg2, currentTimeScale.bg2Offset, resolution * currentTimeScale.bg2Speed * playerY);
    if (currentTimeScale.bg3)
        Render.background(ctx, background, width, height, currentTimeScale.bg3, currentTimeScale.bg3Offset, resolution * currentTimeScale.bg3Speed * playerY);
    ...
    */
    ...
    for (n = 0; n < drawDistance; n++) {
        ...
        Util.project(segment.p1, (playerX * roadWidth) - x, playerY + cameraHeight, position, cameraDepth, width, height, roadWidth);
        Util.project(segment.p2, (playerX * roadWidth) - x - dx, playerY + cameraHeight, position, cameraDepth, width, height, roadWidth);
...
//					}
    ...
    ctx.save();
    if (currentTimeScale.groundColor) {  // draw ground
    ...
    */
    ...
    if (currentTimeScale.roadType == 2) {  // space (0&1) requires indiv road segments to clip flying sprites; sand or water background (2&3) don't show road
        ...
        for (n = (drawDistance - 1); n > 0; n--) {
            ...
            if (seg.drawRoad) {  // sand or water background don't show road
    ...
    for (n = (drawDistance - 1); n > 0; n--) {
        ...
        if (segment.drawRoad && currentTimeScale.roadType == 1) {  // space backgrounds only use indiv segments to overlay flying sprites (bigpoly doesn't work)
...
//											segment.timeMarker);
        ...
        for (i = 0; i < segment.sprites.length; i++) {
            ...
            if (sprite.roadSprite) {
                spriteScale = Util.interpolate(segment.p1.screen.scale, segment.p2.screen.scale, 1);
                spriteX = Util.interpolate(segment.p1.screen.x, segment.p2.screen.x, 1) + (spriteScale * sprite.offset * roadWidth * width / 2);
                spriteY = Util.interpolate(segment.p1.screen.y, segment.p2.screen.y, 1);
                Render.sprite(ctx, width, height, resolution, roadWidth, sprites, sprite.source, spriteScale, spriteX, spriteY, -.5, sprite.offsetY, segment.clip);
            } else {
        ...
        for (i = 0; i < segment.cars.length; i++) {
            ...
            Render.sprite(ctx, width, height, resolution, roadWidth, sprites, car.sprite, spriteScale, spriteX, spriteY, -0.5, car.offsetY, segment.clip , 0, car.percent );
        }
    ...
    //} else {
        ...
        for (var j = 0; j < fragments.length; j++) {
            ...
            ctx.globalAlpha = f.a;
            ctx.drawImage(sprites, fragmentSprite.x, fragmentSprite.y, fragmentSprite.w, fragmentSprite.h, f.x-(f.w/2), f.y-(f.h/2), f.w, f.h);
            ctx.globalAlpha = 1;
...
function addSprite(n, sprite, offset, offsety, spriteOptions) {
    ...
    var spr = { source:sprite, offset:offset, offsetY:offsety, roadSprite:roadSprite, seedSprite:seedSprite, timeJump:timeJump };
...
function timeMarkerVal(timeScale, n) {
    ...
    if (tIncr==500) { //special formating for BCE/CE dates
        //var testYear = ya * 500;
        if (tMarker >= 10000) {
...
// addSprite(120, SPRITES.BILLBOARD02, -.6);
    ...
    for (n = 900; n < segments.length; n += 900) {
        if (Math.random()>=.85) {
            addSprite(n, SPRITES.TIMEJUMPUP, Util.randomInt(-9,9)/10, -1, {timejump:1});
        } else if (Math.random()<=.15) {
            addSprite(n, SPRITES.TIMEJUMPDOWN, Util.randomInt(-9,9)/10, -1, {timejump:-1});
        }
    ...
    for (n = 500; n < segments.length; n += 500) {
        addSprite(n, Util.randomChoice(SPRITES.SEEDS), Util.randomInt(-9,9)/10, -1, {seed:true});
        seedsTotal++;
        //addSprite(n + 500, Util.randomChoice(SPRITES.SEEDS), .2, -1, {seed:true});
    }
    ...
    for (n = 200; n < segments.length; n += skip) {
        ...
        var spr = (timeScale < 2) ? Util.randomChoice(SPRITES.STARS) : (timeScale == 2) ? Util.randomChoice(SPRITES.DESERT) : (timeScale == 3) ? Util.randomChoice(SPRITES.WATER) : Util.randomChoice(SPRITES.PLANTS);
        var offsetY = (timeScale < 2) ? Util.randomChoice([1, -1]) * Math.random() * 6 : spr.oy? spr.oy : -.97;
        // change the factor after Math.random() below to spread the sprites wider or narrower
        addSprite(n, spr, Util.randomChoice([1, -1]) * (1 + Math.random() * 10), offsetY);
    }

    if (timeScale > 3) { // add a bunch more landscape sprites to "regular" levels
...
function resetCars() {
    ...
    for (var n = 0; n < totalCars; n++) {
        ...
        car = { offset:offset, z:z, sprite:sprite, offsetY:offsety, speed:speed };
...
Game.run({
    ...
    keys:[
        ...
        { keys:[KEY.F], mode:'up', action:function () {
...
Dom.on('totalCars', 'change', function (ev) {
    Dom.blur(ev);
    reset({ totalCars:Util.limit(Util.toInt(ev.target.value), Util.toInt(ev.target.getAttribute('min')), Util.toInt(ev.target.getAttribute('max'))) });
    resetRoad();
});
